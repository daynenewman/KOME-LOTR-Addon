...
public final class KOMEForeignConstructionService {
    ...
    public static Decision grant(KOMEWorldData data, String tileId, UUID actor, String granteeFaction, long now) {
        ...
        String owner = tile.projectRulingFaction(), grantee = KOMEAlliance.normalizeFactionKey(granteeFaction);
    ...
    public static Decision canConstruct(KOMEWorldData data, String tileId, UUID builder, String builderFaction) {
        KOMEConquestTile tile = tile(data, tileId); if (tile == null) return Decision.deny("The selected conquest tile is not claimed.");
        String owner = tile.projectRulingFaction(), builderKey = KOMEAlliance.normalizeFactionKey(builderFaction);
    ...
    public static List<KOMEForeignConstructionPermission> currentForTile(KOMEWorldData data, String tileId) {
        KOMEConquestTile tile = tile(data, tileId); if (tile == null) return Collections.emptyList(); String owner = tile.projectRulingFaction();
    ...
    private static KOMEConquestTile tile(KOMEWorldData data, String tileId) { KOMEConquestTile tile = data == null ? null : data.conquestTiles.get(KOMEConquestTile.normalizeId(tileId)); return tile == null || tile.projectRulingFaction().isEmpty() ? null : tile; }
}
