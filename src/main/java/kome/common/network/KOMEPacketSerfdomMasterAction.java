...
public class KOMEPacketSerfdomMasterAction implements IMessage {
    public static final int SERVE=0, REQUEST_DUTY=1, VIEW_DUTY=2, HIGHLIGHT=3, DELIVER_PROVISIONS=4;
    ...
    public static class Handler implements IMessageHandler<KOMEPacketSerfdomMasterAction,IMessage>{public IMessage onMessage(KOMEPacketSerfdomMasterAction message,MessageContext ctx){
        EntityPlayerMP player=ctx.getServerHandler().playerEntity; if(message.action<SERVE||message.action>DELIVER_PROVISIONS){player.addChatMessage(new ChatComponentText("Unknown Serfdom Master action."));return null;}
        ...
        if(message.action==REQUEST_DUTY){KOMESerfdomMasterService.Result result=KOMESerfdomMasterService.requestDuty(player,data,npc);player.addChatMessage(new ChatComponentText(result.success?"You received your next Serfdom duty. See the progression book for details.":sameDayFlavor(result.reason)));if(result.success)kome.common.data.KOMEProgressionAutoCompleter.syncPlayer(player,data.getProgression(KOMEReflection.getEntityUUID(player)));sendMenu(player,npc);}
        else if(message.action==HIGHLIGHT)KOMESerfdomMasterService.highlightMaster(player,data);
        else if(message.action==DELIVER_PROVISIONS){kome.common.data.KOMESerfKnightDutyType active=kome.common.data.KOMESerfKnightDutyType.forKey(state.getActiveAssignmentKind());kome.common.data.KOMESerfProvisioningAssignment assignment=active==kome.common.data.KOMESerfKnightDutyType.PROVISIONING?kome.common.data.KOMESerfProvisioningAssignment.readFromNBT(state.getDuty(active).getAssignmentData()):null;if(assignment==null){player.addChatMessage(new ChatComponentText("No valid Provisioning duty is active."));return null;}int delivered=kome.common.data.KOMESerfProvisioningService.deliver(assignment,player.inventory);if(delivered<=0){player.addChatMessage(new ChatComponentText("You have no matching provisions to deliver."));return null;}state.setDutyAssignmentData(active,assignment.writeToNBT());if(assignment.complete())kome.common.data.KOMESerfKnightService.completeDuty(state,active);data.markDirty();player.inventoryContainer.detectAndSendChanges();kome.common.data.KOMEProgressionAutoCompleter.syncPlayer(player,data.getProgression(KOMEReflection.getEntityUUID(player)));player.addChatMessage(new ChatComponentText(assignment.complete()?"Provisioning duty complete.":"Delivered provisions. Remaining quotas are in your progression book."));sendMenu(player,npc);}
