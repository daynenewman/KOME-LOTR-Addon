...
public class KOMEDiplomacyServiceTest {
    ...
    @Test public void kingRequestAndReceivingKingAcceptanceAreBilateral() {
        KOMEWorldData data=new KOMEWorldData("d"); UUID g=UUID.randomUUID(),r=UUID.randomUUID(); KOMERulerService.assignRuler(data,"gondor",g,"G"); KOMERulerService.assignRuler(data,"rohan",r,"R");
    ...
    @Test public void requestsRequireKingsAndReceiverKing() {
        KOMEWorldData data=new KOMEWorldData("d"); UUID g=UUID.randomUUID(); KOMERulerService.assignRuler(data,"gondor",g,"G");
    ...
    @Test public void canonicalPersistenceRoundTripsPendingAndAcceptedRecords() {
        KOMEWorldData data=new KOMEWorldData("d"); UUID g=UUID.randomUUID(),r=UUID.randomUUID(); KOMERulerService.assignRuler(data,"gondor",g,"G"); KOMERulerService.assignRuler(data,"rohan",r,"R"); KOMEDiplomacyService.requestIncrease(data,"gondor","rohan",KOMEDiplomacyRelation.FRIENDS,g,42L);
        ...
        NBTTagCompound malformed=new NBTTagCompound(); malformed.setString("FactionA","gondor");malformed.setString("FactionB","gondor");malformed.setString("Relation","allies"); KOMEWorldData bad=new KOMEWorldData("b"); NBTTagCompound root=new NBTTagCompound(); new KOMEWorldData("canonical-fixture").writeToNBT(root); root.setInteger(KOMEWorldData.KOME_DATA_SCHEMA_KEY,KOMEWorldData.KOME_DATA_SCHEMA_VERSION); root.setInteger("BuildDataSchemaVersion",KOMEWorldData.BUILD_DATA_SCHEMA_VERSION); root.setTag("Builds",new net.minecraft.nbt.NBTTagList()); root.setInteger("FactionPopulationDataSchemaVersion",KOMEWorldData.FACTION_POPULATION_DATA_SCHEMA_VERSION); net.minecraft.nbt.NBTTagList list=new net.minecraft.nbt.NBTTagList();list.appendTag(malformed);root.setTag("CanonicalDiplomacyRecords",list);bad.readFromNBT(root);assertTrue(bad.canonicalDiplomacyRecords.isEmpty());
    }
}
