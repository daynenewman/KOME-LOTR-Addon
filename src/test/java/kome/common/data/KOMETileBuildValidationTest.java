...
public class KOMETileBuildValidationTest {
    ...
    private static String saved(KOMEWorldData data) {
        // Schema 4 also validates capitals while serializing. Supply geometry only for
        // this read-only comparison, never for the rejected Build operation itself.
        boolean unavailable = !KOMETileWorldResolver.INSTANCE.snapshot().isPresent();
        if (unavailable) KOMETileWorldResolver.INSTANCE.publish(KOMETileTestResources.real());
        try {
    ...
    @Test public void existingPersistedBuildsOutsideCoverageAreNotRevalidatedOrModified() {
        ...
        try {
            ...
            fail("Schema-4 capitals require available geometry during whole-world loading");
        } catch (IllegalStateException expected) {
            assertTrue(expected.getMessage(), expected.getMessage().contains("FactionCapitals"));
            assertTrue(expected.getMessage(), expected.getMessage().contains("INVALID_SNAPSHOT"));
            assertTrue(reloadedWorld.isWriteBlocked());
        }
        // Build records themselves still load unchanged without spatial revalidation.
        KOMEPlayerBuild withoutGeometry = new KOMEPlayerBuild();
        withoutGeometry.readFromNBT(serialized);
        assertEquals(serialized.toString(), withoutGeometry.writeToNBT().toString());
        KOMETileWorldResolver.INSTANCE.publish(KOMETileTestResources.real());
        reloadedWorld = new KOMEWorldData("compatible-reload");
        reloadedWorld.readFromNBT(entireWorld);
