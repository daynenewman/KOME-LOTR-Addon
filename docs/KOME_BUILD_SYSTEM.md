...
Hours are represented canonically as integer half-hours. `KOMEBuildPopulationService` rejects negative, NaN, infinite, or non-half-hour input. The default conversion is 5 population per half-hour (10 per hour), stored as `buildPopulationPerHalfHour` and configurable through `/build config populationPerHalfHour <value>`.
...
- **Delete Build**: current manager or operator; always requires GUI confirmation.
- **Destroy Enemy Build**: recognized king of the tile controller or operator; only when the Build owner is currently hostile, the controller owns the tile now, and the tile is part of that controller's original/default homeland. It cannot remove Neutral/Friendly/Allied Builds or enemy Builds in foreign conquered territory.
...
The Build list supports multiple projects and owners in one tile. Detail shows owner, builder, manager, coordinates, status, hours, generated and committed population, controller usability, pending count, and contribution records. Normal GUI flows provide creation, half-hour contribution, pending review, approval/rejection, rename, contribution removal, deletion, and eligible hostile destruction. Destructive actions use `KOMEGuiConfirmationDialog`.

Creation uses the selected tile/current world coordinates supplied by the authoritative open-screen context. The owner selector contains only server-provided eligible factions and explains the difference between credit and population ownership.
