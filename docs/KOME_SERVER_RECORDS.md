...
Alliance detail is also server-authored. `TRACK` records carry each faction side's rolled item, delivered amount, activity and Military population progress, completion/waiver/grace state, effective benefit, mutation flag, and exact rejection reason. The client does not reverse contributor and receiver to infer these values.
