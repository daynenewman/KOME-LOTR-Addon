...
reviewed dev `15ff1340b24d14b563f2cdb6ba3d7dc96be7d6c6` in the separate integration
worktree. Capital definition, relocation and deployment callers now use explicit
dimensions and typed exact results. Dev capital lifecycle/schema requirements,
defensive-gate cleanup and public capital summaries coexist with the tile HUD,
Build guards, deliberate form confirmation and authoritative borders. Owner-list
population uses one idempotent server authorization path.

Validation of the combined source: 754 focused KOME tests passed; one clean
`test build` discovered 953 tests, with 951 passed, two existing Windows symlink
skips and zero failures/errors. The original 6,055 gameplay expectations pass.
The approved mask and gameplay-default resources are byte-identical to the tile
checkpoint. Production JAR SHA-256:
`ee6460da208bef1808dcffa3c8148c181fe24a25b7c0cfe39c2fa433b16ce020`.

A separate loopback-only, online-mode=true disposable fresh world completed two
startup/save/stop cycles with 24/24 capitals READY and six resolver controls per
cycle. All saved root fields except the expected appended CONFIG/WORLD_BOUND
audit entry matched after restart, including capital anchors, 621 tiles and 621
waypoint defaults. Fresh Builds and movements were empty; populated-record
compatibility is covered by automated tests, not this runtime exercise. The
disposable server is stopped and the established validation installation was
not changed. Legacy Forge signature/version-check warnings remained nonfatal.

Dev requires root schema 4, Build schema 3 and capital schema 1 with all 24
records. There is **no supported root-3 migration**. Missing geometry blocks
capital validation on whole-world loading; individual existing Build records
remain loadable without spatial revalidation. No schema bypass or migration was
added. Older validation worlds must not be opened with this candidate.

The user's Rhun T401 -> T442 acceptance below applies to the **earlier tile
checkpoint only**, not the combined runtime. Combined-client manual checks remain:
public capital summaries, non-operator Build create/cancel/rejection, capital
deployment/formation, multi-zoom border/hover alignment, river width and HUD
lifecycle. Use a separate compatible client/fresh world after authorization;
preserve the established installation. No additional geography was approved.
